...
import preprocess
from model import build_model, build_multimodal_modelv5
...
# Preprocesar metadatos del usuario (incluye creación y métricas)
data = preprocess.preprocess_user_metrics(data)

# Medir tiempo al terminar toda la preparación
end_time = time.time()
elapsed_time = end_time - start_time
print(f"Tiempo de carga y preprocesamiento (v4): {elapsed_time:.2f} segundos")

# Separar características y etiquetas
x_text = data["tweet_sample_clean"].values
# Variables de entrada para el modelo (8 metadatos)
x_numeric = data[[
    "followers_count_scaled",
    "following_count_scaled",
    "tweet_count_scaled",
    "user_age_days_scaled",
    "followers_following_ratio_scaled",
    "tweets_per_day_scaled",
    "verified",
    "protected",
    "has_profile_image",
    "has_url"
...
model, text_vectorizer = build_multimodal_modelv5(n_numeric_features=x_numeric.shape[1])
...
save_results_completo(
    file_path="../../resultados/v5_completo.txt",
    model=model,
    loss=loss,
    accuracy=accuracy,
