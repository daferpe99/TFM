...
def preprocess_user_metrics(data):
    ...
    data['user_age_days_scaled'] = scaler.fit_transform(data[['user_age_days']])

    # Derived metrics
    data['followers_following_ratio'] = data.apply(
        lambda row: row['followers_count'] / row['following_count'] if row['following_count'] > 0 else 0, axis=1)
    data['tweets_per_day'] = data.apply(
        lambda row: row['tweet_count'] / row['user_age_days'] if row['user_age_days'] > 0 else 0, axis=1)

    # Scaling all numeric fields
    cols_to_scale = [
        "followers_count", "following_count", "tweet_count",
        "followers_following_ratio", "tweets_per_day"
    ]
